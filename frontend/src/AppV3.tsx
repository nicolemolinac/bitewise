...
function imageFor(m:Meal){return imageProxy(m)}
function imageFallback(e:React.SyntheticEvent<HTMLImageElement>){e.currentTarget.style.display='none'}
...
export default function AppV3(){
  ...
  return <div className="min-h-screen bg-[#f7f4ee] text-[#17211b]">
    ...
    {altItem&&<div className="fixed inset-0 z-50 bg-black/40 p-4" onClick={()=>setAltItem(null)}><div className="mx-auto mt-16 max-w-xl rounded-3xl bg-white p-5" onClick={e=>e.stopPropagation()}><div className="flex items-center justify-between"><div className="font-black">Alternatives for {altItem.ingredient}</div><button onClick={()=>setAltItem(null)}><X/></button></div><div className="mt-4 space-y-2">{alternatives.map(p=><button key={p.id||p.name_original} onClick={()=>chooseAlternative(p)} className="flex w-full items-center justify-between rounded-xl bg-[#f5f4ef] p-3 text-left"><span className="font-black">{p.name_original}</span><span className="text-sm">{money(p.price)}</span></button>)}</div></div></div>}
  </div>
}

function MealCard({meal,open,like,skip,picked}:{meal:Meal;open:()=>void;like:()=>void;skip:()=>void;picked:boolean}){
  return <article className="group overflow-hidden rounded-[24px] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><button onClick={open} className="block w-full text-left"><div className="aspect-[4/3] overflow-hidden bg-[#e8e5dc]"><img src={imageFor(meal)} alt={meal.name} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.04]" loading="lazy" onError={imageFallback}/></div><div className="p-4"><div className="flex items-start justify-between gap-2"><h3 className="font-black leading-tight">{meal.name}</h3><span className="shrink-0 rounded-full bg-[#dff2df] px-2 py-1 text-[10px] font-black">≈{caloriesOf(meal)} kcal</span></div><div className="mt-2 text-xs text-[#18392b]/45">{meal.time} min · {meal.cuisine} · {money(meal.cost)}</div>{meal.is_concept&&<div className="mt-2 text-[10px] font-black uppercase tracking-[.12em] text-[#18392b]/35">Recipe generated only if you pick it</div>}</div></button><div className="flex gap-2 px-4 pb-4"><button onClick={skip} className="rounded-xl border border-[#18392b]/10 p-2"><X size={14}/></button><button onClick={like} className="flex-1 rounded-xl bg-[#18392b] px-3 py-2 text-xs font-black text-white">{picked?'Remove':'♥ I’d eat this'}</button></div></article>
}
