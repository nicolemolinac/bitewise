...
function imageProxy(m:Meal){
  const q=new URLSearchParams({name:m.name||'',description:m.description||'',tags:(m.tags||[]).join(',')});
  return `${API}/meal-image?${q.toString()}`;
}
function imageFor(m:Meal){return m.image||imageProxy(m)}
function imageFallback(e:React.SyntheticEvent<HTMLImageElement>,m:Meal){const img=e.currentTarget;const fallback=imageProxy({...m,image:''});if(img.src!==fallback)img.src=fallback}
...
function portionFactor(profile:Profile){const body=Math.sqrt((Math.max(profile.weightKg,35)/70)*(Math.max(profile.heightCm,140)/170));return clamp(body*profile.portionPreference,0.65,1.4)}
...
export default function AppV3(){
  ...
  const weeklyPlan=useMemo(()=>{const groups:Record<MealType,Meal[]>={breakfast:[],lunch:[],dinner:[],dessert:[],snack:[]};liked.forEach(m=>groups[mealTypeOf(m)].push(m));return Array.from({length:7},(_,day)=>({day,slots:mealTypes.map(t=>({type:t.id,meal:groups[t.id].length?groups[t.id][day%groups[t.id].length]:null} as Slot)).filter(x=>x.meal)}))},[liked]);
  const factor=portionFactor(profile);const todayIndex=(new Date().getDay()+6)%7;const todaySlots=weeklyPlan[todayIndex]?.slots||[];
  const weeklyStats=useMemo(()=>{let cost=0,calories=0,mealCount=0;weeklyPlan.forEach((d,idx)=>d.slots.forEach(s=>{if(!s.meal)return;const portions=dayContexts[idx]?.people||1;cost+=s.meal.cost*factor*portions;calories+=caloriesOf(s.meal)*factor;mealCount++}));return {cost,avgCalories:mealCount?Math.round(calories/7):0,mealCount}},[weeklyPlan,dayContexts,factor]);

  async function generateAI(prompt:string){setLoading(true);setError('');try{const full=`${prompt}. Meal type: ${mealType}. Available appliances: ${appliances.join(', ')||'basic kitchen'}. User portion factor is ${factor.toFixed(2)}. Prefer the easiest method, minimal cleanup, and products likely available in the current REWE catalog.`;const d=await api('/ai/meals',{method:'POST',body:JSON.stringify({prompt:full})});const fresh:Meal[]=d.meals||[];if(!fresh.length)throw new Error('No meal ideas were generated. Check GEMINI_API_KEY in backend/.env.');setMeals(current=>[...fresh,...current.filter(x=>!fresh.some(n=>n.id===x.id))]);setFilter('all')}catch(e:any){setError(e.message)}finally{setLoading(false)}}
  async function react(meal:Meal,action:'like'|'skip'|'dislike'){if(action==='like'&&meal.is_concept){setLoading(true);setError('');try{const d=await api('/ai/recipe',{method:'POST',body:JSON.stringify({concept:meal,context:meal.discovery_prompt||brain})});const recipe:Meal=d.meal;if(!recipe?.id)throw new Error('Bitewise could not build the selected recipe.');try{await api('/events',{method:'POST',body:JSON.stringify({meal_id:recipe.id,action:'like'})})}catch{}setLiked(x=>x.some(a=>a.id===recipe.id)?x:[...x,recipe]);setSkipped(x=>[...new Set([...x,meal.id])])}catch(e:any){setError(e.message)}finally{setLoading(false)}return}if(!meal.is_concept){try{await api('/events',{method:'POST',body:JSON.stringify({meal_id:meal.id,action})})}catch{}}if(action==='like')setLiked(x=>x.some(a=>a.id===meal.id)?x:[...x,meal]);else setSkipped(x=>[...new Set([...x,meal.id])])}
  function servingTotals(){const totals:Record<string,number>={};weeklyPlan.forEach((d,idx)=>d.slots.forEach(s=>{if(s.meal)totals[s.meal.id]=(totals[s.meal.id]||0)+factor*(dayContexts[idx]?.people||1)}));if(!Object.keys(totals).length)liked.forEach(m=>totals[m.id]=factor);return totals}
  async function makeBasket(nextStrategy:Strategy=strategy){if(!liked.length)return;setLoading(true);setError('');try{const servings=servingTotals();const d=await api('/shopping',{method:'POST',body:JSON.stringify({meal_ids:Object.keys(servings),servings,owned,mode:nextStrategy})});setBasket(d.items||d.basket||[]);setSummary(d);setTab('basket')}catch(e:any){setError(e.message)}finally{setLoading(false)}}
  ...
  async function toggleOwned(ingredient:string){const next=owned.includes(ingredient)?owned.filter(x=>x!==ingredient):[...owned,ingredient];setOwned(next);try{await api('/shopping/state',{method:'PUT',body:JSON.stringify({ingredient,state:next.includes(ingredient)?'owned':'needed'})})}catch{}}
  async function openAlternatives(item:BasketItem){setAltItem(item);try{const d=await api(`/products/search?q=${encodeURIComponent(item.ingredient)}`);setAlternatives(d.products||[])}catch(e:any){setError(e.message)}}
  ...
  function addExtra(p:Product){const id=`${extraKind}-${p.id||p.name_original}`;if(extras.some(x=>x.id===id))return;setExtras(x=>[...x,{id,kind:extraKind,label:p.name_original,product:p,dailyUse:extraKind==='beverage'?1:0,onHand:0}])}
  ...
  return <div className="min-h-screen bg-[#f7f4ee] text-[#17211b]">
    <header className="sticky top-0 z-40 border-b border-[#17211b]/8 bg-[#f7f4ee]/92 backdrop-blur-xl"><div className="mx-auto flex max-w-[1500px] items-center justify-between px-5 py-3"><button onClick={()=>setTab('discover')} className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#18392b] text-white"><UtensilsCrossed size={17}/></span><div className="text-left"><div className="font-black">Bitewise</div><div className="text-[9px] font-bold uppercase tracking-[.22em] text-[#18392b]/45">Eat well. Think less.</div></div></button><nav className="hidden gap-1 rounded-full bg-white p-1 shadow-sm xl:flex">{(['discover','picks','plan','today','basket','extras'] as Tab[]).map(t=><button key={t} onClick={()=>setTab(t)} className={cx('rounded-full px-4 py-2 text-xs font-black capitalize transition',tab===t?'bg-[#18392b] text-white':'text-[#18392b]/50 hover:bg-[#edf3ee] hover:text-[#18392b]')}>{t}{t==='picks'&&liked.length?` ${liked.length}`:''}</button>)}</nav><div className="flex gap-1"><button onClick={()=>setTab('pantry')} className="rounded-full bg-white p-2.5 shadow-sm"><Home size={16}/></button><button onClick={()=>setTab('settings')} className="rounded-full bg-white p-2.5 shadow-sm"><Settings size={16}/></button></div></div></header>
    <main className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6">
      ...
      {tab==='discover'&&<><div className="grid gap-3 rounded-[26px] bg-[#18392b] p-5 text-white lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="text-xs font-black uppercase tracking-[.18em] text-white/45">AI food decision engine</div><h1 className="mt-1 text-2xl font-black">What do you feel like eating?</h1><p className="mt-1 text-sm text-white/55">Browse lightweight ideas first. Bitewise creates the full recipe only when you pick one.</p></div><div className="text-right"><div className="text-xs text-white/40">Adaptive portion</div><div className="text-xl font-black">{factor.toFixed(2)}×</div></div></div><div className="mt-4 flex gap-2 overflow-x-auto pb-1">{mealTypes.map(t=><button key={t.id} onClick={()=>setMealType(t.id)} className={cx('shrink-0 rounded-full px-4 py-2 text-sm font-black',mealType===t.id?'bg-[#dff2df] text-[#173b2d]':'bg-white text-[#173b2d]/55')}>{t.emoji} {t.label}</button>)}</div><div className="mt-3 flex flex-col gap-2 rounded-2xl bg-white p-3 shadow-sm sm:flex-row"><input value={brain} onChange={e=>setBrain(e.target.value)} onKeyDown={e=>e.key==='Enter'&&brain.trim()&&generateAI(brain)} className="min-w-0 flex-1 rounded-xl bg-[#f3f1eb] px-4 py-3 text-sm outline-none" placeholder="Fancy but easy tonight · cheap breakfast · use what I have..."/><button onClick={()=>brain.trim()&&generateAI(brain)} className="rounded-xl bg-[#18392b] px-5 py-3 text-sm font-black text-white">{loading?'Thinking…':'Generate'}</button></div><div className="mt-4 flex gap-2 overflow-x-auto pb-1">{cuisines.map(c=><button key={c} onClick={()=>setFilter(c)} className={cx('shrink-0 rounded-full border px-3 py-1.5 text-xs font-black capitalize',filter===c?'border-[#18392b] bg-[#18392b] text-white':'border-[#18392b]/10 bg-white text-[#18392b]/55')}>{c.replace('-',' ')}</button>)}</div>{!visible.length?<div className="mt-5 rounded-[28px] bg-white p-10 text-center"><ChefHat className="mx-auto text-[#18392b]/20"/><div className="mt-3 font-black">No exact matches yet</div><button onClick={()=>generateAI(`${filter==='all'?'varied':filter} ${mealType} ideas`)} className="mt-4 rounded-full bg-[#18392b] px-5 py-2.5 text-sm font-black text-white">Generate matching meals</button></div>:<div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{visible.map(m=><MealCard key={m.id} meal={m} open={()=>!m.is_concept&&setDetail(m)} like={()=>react(m,'like')} skip={()=>react(m,'skip')} picked={false}/>)}</div>}</>}
      {tab==='picks'&&<><Title title="Your picks" sub="Only selected ideas become full recipes. Portions adapt to your profile and each day's guest count."/><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{liked.map(m=><MealCard key={m.id} meal={m} open={()=>setDetail(m)} like={()=>setLiked(x=>x.filter(a=>a.id!==m.id))} skip={()=>setLiked(x=>x.filter(a=>a.id!==m.id))} picked/>)}</div>{liked.length>0&&<button onClick={()=>makeBasket()} className="mt-6 rounded-2xl bg-[#18392b] px-5 py-3 text-sm font-black text-white"><ShoppingBasket size={15} className="mr-2 inline"/>Build adaptive weekly basket</button>}</>}
      {tab==='plan'&&<><div className="flex flex-wrap items-end justify-between gap-3"><Title title="Calendar" sub="Week, month and year views are projected from your current recurring meal plan."/><div className="flex rounded-full bg-white p-1 shadow-sm">{(['week','month','year'] as CalendarView[]).map(v=><button key={v} onClick={()=>setCalendarView(v)} className={cx('rounded-full px-4 py-2 text-xs font-black capitalize',calendarView===v?'bg-[#18392b] text-white':'text-[#18392b]/45')}>{v}</button>)}</div></div><Stats cost={weeklyStats.cost} calories={weeklyStats.avgCalories} count={weeklyStats.mealCount}/>{calendarView==='week'?<div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{weeklyPlan.map((d,i)=><DayCard key={i} day={d} index={i} context={dayContexts[i]} update={p=>updateDay(i,p)} open={setDetail}/>)}</div>:calendarView==='month'?<MonthView weekly={weeklyStats}/>:<YearView weekly={weeklyStats}/>}</>}
      {tab==='today'&&<><Title title="Today" sub={`${dayContexts[todayIndex]?.people||1} person${(dayContexts[todayIndex]?.people||1)>1?'s':''}${dayContexts[todayIndex]?.occasion?` · ${dayContexts[todayIndex].occasion}`:''}`}/><div className="mt-5 space-y-4">{todaySlots.length?todaySlots.map(s=><div key={s.type} className="grid overflow-hidden rounded-[26px] bg-white shadow-sm md:grid-cols-[280px_1fr]"><img src={imageFor(s.meal!)} onError={e=>imageFallback(e,s.meal!)} className="h-56 w-full object-cover md:h-full"/><div className="p-6"><div className="text-xs font-black uppercase tracking-[.16em] text-[#18392b]/35">{s.type}</div><div className="mt-1 text-2xl font-black">{s.meal?.name}</div><div className="mt-2 text-sm text-[#18392b]/45">≈ {Math.round(caloriesOf(s.meal!)*factor)} kcal for your portion · {s.meal?.time} min</div><RecipeSteps meal={s.meal!} appliances={appliances}/></div></div>):<Empty text="No meals planned for today yet."/>}</div></>}
      ...
      {tab==='extras'&&<><Title title="Drinks & extras" sub="Beverages and household items stay outside the meal planner but inside your shopping view."/><div className="mt-4 flex gap-2"><button onClick={()=>setExtraKind('beverage')} className={cx('rounded-full px-4 py-2 text-sm font-black',extraKind==='beverage'?'bg-[#18392b] text-white':'bg-white')}>🥤 Beverages</button><button onClick={()=>setExtraKind('non-food')} className={cx('rounded-full px-4 py-2 text-sm font-black',extraKind==='non-food'?'bg-[#18392b] text-white':'bg-white')}>🧻 Non-food</button></div><div className="mt-4 flex gap-2"><input value={extraQuery} onChange={e=>setExtraQuery(e.target.value)} className="flex-1 rounded-xl bg-white px-4 py-3" placeholder="Search REWE products…"/><button onClick={searchExtras} className="rounded-xl bg-[#18392b] px-4 text-white"><Search size={16}/></button></div><div className="mt-4 grid gap-3 md:grid-cols-2">{extraResults.slice(0,8).map(p=><button key={p.id||p.name_original} onClick={()=>addExtra(p)} className="rounded-2xl bg-white p-4 text-left"><div className="font-black">{p.name_original}</div><div className="text-xs text-[#18392b]/40">{money(p.price)}</div></button>)}</div><div className="mt-6 space-y-3">{extras.map(e=><div key={e.id} className="rounded-2xl bg-white p-4"><div className="font-black">{e.label}</div><div className="mt-3 grid grid-cols-2 gap-3"><label className="text-xs">Daily use<input type="number" value={e.dailyUse} onChange={x=>updateExtra(e.id,{dailyUse:Number(x.target.value)})} className="mt-1 w-full rounded-lg bg-[#f3f1eb] p-2"/></label><label className="text-xs">On hand<input type="number" value={e.onHand} onChange={x=>updateExtra(e.id,{onHand:Number(x.target.value)})} className="mt-1 w-full rounded-lg bg-[#f3f1eb] p-2"/></label></div></div>)}</div></>}
      {tab==='pantry'&&<><Title title="Pantry" sub="What you already own reduces the basket automatically."/><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{pantry.map((p:any)=><div key={p.id} className="rounded-2xl bg-white p-4"><div className="font-black capitalize">{p.ingredient}</div><div className="text-xs text-[#18392b]/40">{p.quantity} {p.unit} · {p.status}</div></div>)}</div></>}
    ...
    {detail&&<div className="fixed inset-0 z-50 overflow-y-auto bg-black/55 p-4" onClick={()=>setDetail(null)}><div className="mx-auto mt-8 max-w-3xl overflow-hidden rounded-[30px] bg-white shadow-2xl" onClick={e=>e.stopPropagation()}><img src={imageFor(detail)} onError={e=>imageFallback(e,detail)} className="h-64 w-full object-cover"/><div className="p-6"><div className="flex justify-between gap-4"><div><div className="text-xs font-black uppercase text-[#18392b]/35">{mealTypeOf(detail)} · ≈ {Math.round(caloriesOf(detail)*factor)} kcal your portion</div><h2 className="mt-1 text-3xl font-black">{detail.name}</h2></div><button onClick={()=>setDetail(null)}><X/></button></div><RecipeSteps meal={detail} appliances={appliances}/><div className="mt-5"><div className="text-xs font-black uppercase text-[#18392b]/35">Ingredients</div><div className="mt-2 flex flex-wrap gap-2">{detail.ingredients.map(([n,q,u])=><span key={n} className="rounded-full bg-[#f3f1eb] px-3 py-1.5 text-xs font-bold">{n} · {Math.round(q*factor*100)/100} {u}</span>)}</div></div></div></div></div>}
    {altItem&&<div className="fixed inset-0 z-50 bg-black/50 p-4" onClick={()=>setAltItem(null)}><div className="mx-auto mt-20 max-w-xl rounded-3xl bg-white p-5" onClick={e=>e.stopPropagation()}><div className="flex items-center justify-between"><div className="font-black">Alternatives for {altItem.ingredient}</div><button onClick={()=>setAltItem(null)}><X/></button></div><div className="mt-4 space-y-2">{alternatives.slice(0,10).map(p=><button key={p.id||p.name_original} onClick={()=>chooseAlternative(p)} className="w-full rounded-xl bg-[#f5f4ef] p-3 text-left"><div className="font-black">{p.name_original}</div><div className="text-xs text-[#18392b]/40">{money(p.price)}</div></button>)}</div></div></div>}
  </div>
}

function MealCard({meal,open,like,skip,picked}:{meal:Meal;open:()=>void;like:()=>void;skip:()=>void;picked:boolean}){return <article className="group overflow-hidden rounded-[24px] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><button onClick={open} className="block w-full text-left"><div className="aspect-[4/3] overflow-hidden bg-[#e8e5dc]"><img src={imageFor(meal)} alt={meal.name} onError={e=>imageFallback(e,meal)} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.04]" loading="lazy"/></div><div className="p-4"><div className="flex items-start justify-between gap-2"><h3 className="font-black leading-tight">{meal.name}</h3><span className="shrink-0 rounded-full bg-[#dff2df] px-2 py-1 text-[10px] font-black">≈{caloriesOf(meal)} kcal</span></div><div className="mt-2 text-xs text-[#18392b]/45">{meal.time} min · {meal.cuisine} · {money(meal.cost)}</div>{meal.is_concept&&<div className="mt-2 text-[10px] font-black uppercase tracking-[.12em] text-[#18392b]/35">Recipe generated only if you pick it</div>}</div></button><div className="flex gap-2 px-4 pb-4"><button onClick={skip} className="rounded-xl border border-[#18392b]/10 p-2"><X size={14}/></button><button onClick={like} className="flex-1 rounded-xl bg-[#18392b] px-3 py-2 text-xs font-black text-white">{picked?'Remove':'♥ I’d eat this'}</button></div></article>}
...
function DayCard({day,index,context,update,open}:{day:{day:number;slots:Slot[]};index:number;context:DayContext;update:(p:Partial<DayContext>)=>void;open:(m:Meal)=>void}){return <div className="rounded-[22px] bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><div><div className="text-xs font-black uppercase text-[#18392b]/35">Day {index+1}</div><div className="font-black">{dayNames[index]}</div></div><CalendarDays size={16} className="text-[#18392b]/25"/></div><div className="mt-3 grid grid-cols-[100px_1fr] gap-2"><label className="text-[10px] font-bold text-[#18392b]/40">People<input type="number" min="1" max="12" value={context.people} onChange={e=>update({people:Math.max(1,Number(e.target.value))})} className="mt-1 w-full rounded-lg bg-[#f3f1eb] px-2 py-2 text-sm text-[#18392b]"/></label><label className="text-[10px] font-bold text-[#18392b]/40">Context<input value={context.occasion} onChange={e=>update({occasion:e.target.value})} placeholder="Dinner party…" className="mt-1 w-full rounded-lg bg-[#f3f1eb] px-2 py-2 text-sm text-[#18392b]"/></label></div><div className="mt-3 space-y-2">{day.slots.map(s=><button key={s.type} onClick={()=>s.meal&&open(s.meal)} className="flex w-full gap-2 rounded-xl bg-[#f7f5ef] p-2 text-left hover:bg-[#edf3ee]">{s.meal&&<img src={imageFor(s.meal)} onError={e=>imageFallback(e,s.meal!)} className="h-12 w-14 rounded-lg object-cover"/>}<div className="min-w-0"><div className="text-[9px] font-black uppercase text-[#18392b]/35">{s.type}</div><div className="truncate text-xs font-black">{s.meal?.name}</div></div></button>)}{!day.slots.length&&<div className="py-4 text-xs text-[#18392b]/35">No meals selected.</div>}</div></div>}
