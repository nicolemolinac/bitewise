...
type Meal = {
  ...
  image_page?:string; photographer?:string;
};
...
export default function AppV3(){
  ...
  async function attachFreePhotos(fresh:Meal[]){
    const enriched = await Promise.all(fresh.map(async meal=>{
      try{
        const q=new URLSearchParams({name:meal.name,meal_type:mealTypeOf(meal),tags:(meal.tags||[]).join(',')});
        const photo=await api(`/meal-photo?${q.toString()}`);
        if(photo?.image)return {...meal,image:photo.image,image_page:photo.image_page||'',photographer:photo.photographer||''};
      }catch{}
      return {...meal,image:''};
    }));
    setMeals(enriched);
  ...
  return <div className="min-h-screen bg-[#f7f4ee] text-[#17211b]">
    ...
    <main className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6">
      ...
      {tab==='today'&&<><Title title="Today" sub={`${dayContexts[todayIndex]?.people||1} person${(dayContexts[todayIndex]?.people||1)>1?'s':''}${dayContexts[todayIndex]?.occasion?` · ${dayContexts[todayIndex].occasion}`:''}`}/><div className="mt-5 space-y-4">{todaySlots.length?todaySlots.map(s=><div key={s.type} className="grid overflow-hidden rounded-[26px] bg-white shadow-sm md:grid-cols-[280px_1fr]">{s.meal?.image?<img src={s.meal.image} className="h-56 w-full object-cover md:h-full"/>:<div className="h-56 bg-[#e8e5dc] md:h-full"/>}<div className="p-6"><div className="text-xs font-black uppercase tracking-[.16em] text-[#18392b]/35">{s.type}</div><div className="mt-1 text-2xl font-black">{s.meal?.name}</div><div className="mt-2 text-sm text-[#18392b]/45">≈ {Math.round(caloriesOf(s.meal!)*factor)} kcal for your portion · {s.meal?.time} min</div><RecipeSteps meal={s.meal!} appliances={appliances}/></div></div>):<Empty text="No meals planned for today yet."/>}</div></>}
      ...
      {tab==='extras'&&<><Title title="Drinks & extras" sub="Beverages and household items stay outside the meal planner but inside your shopping view."/><div className="mt-4 flex gap-2"><button onClick={()=>setExtraKind('beverage')} className={cx('rounded-full px-4 py-2 text-sm font-black',extraKind==='beverage'?'bg-[#18392b] text-white':'bg-white')}>🥤 Beverages</button><button onClick={()=>setExtraKind('non-food')} className={cx('rounded-full px-4 py-2 text-sm font-black',extraKind==='non-food'?'bg-[#18392b] text-white':'bg-white')}>🧻 Non-food</button></div><div className="mt-3 flex gap-2"><input value={extraQuery} onChange={e=>setExtraQuery(e.target.value)} className="flex-1 rounded-xl bg-white px-3 py-2.5" placeholder="Search REWE catalog…"/><button onClick={searchExtras} className="rounded-xl bg-[#18392b] px-4 py-2 text-sm font-black text-white">Search</button></div><div className="mt-4 grid gap-2 md:grid-cols-2">{extraResults.map(p=><button key={String(p.id)+p.name_original} onClick={()=>addExtra(p)} className="rounded-xl bg-white p-3 text-left"><div className="font-black">{p.name_original}</div><div className="text-xs text-[#18392b]/40">{money(p.price)}</div></button>)}</div><div className="mt-5 grid gap-3 lg:grid-cols-2">{extras.map(e=><div key={e.id} className="rounded-[22px] bg-white p-4"><div className="font-black">{e.label}</div><div className="mt-3 grid grid-cols-2 gap-2"><label className="text-xs">Daily use<input type="number" value={e.dailyUse} onChange={x=>updateExtra(e.id,{dailyUse:Number(x.target.value)})} className="mt-1 w-full rounded-lg bg-[#f3f1eb] p-2"/></label><label className="text-xs">On hand<input type="number" value={e.onHand} onChange={x=>updateExtra(e.id,{onHand:Number(x.target.value)})} className="mt-1 w-full rounded-lg bg-[#f3f1eb] p-2"/></label></div></div>)}</div></>}

      {tab==='pantry'&&<><Title title="Pantry" sub="What you already have reduces the basket automatically."/><div className="mt-5 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{pantry.map((p:any)=><div key={p.id} className="rounded-xl bg-white p-3"><div className="font-black capitalize">{p.ingredient}</div><div className="text-xs text-[#18392b]/40">{p.quantity} {p.unit}</div></div>)}</div></>}
      {tab==='settings'&&<><Title title="Your profile" sub="Used only to estimate practical portion sizes. This is not medical or nutrition advice."/><div className="mt-5 grid gap-5 lg:grid-cols-2"><div className="rounded-[24px] bg-white p-5 shadow-sm"><div className="font-black">Portion sizing</div><div className="mt-4 grid grid-cols-2 gap-3"><label className="text-xs font-bold text-[#18392b]/50">Height (cm)<input type="number" value={profile.heightCm} onChange={e=>setProfile({...profile,heightCm:Number(e.target.value)})} className="mt-1 w-full rounded-xl bg-[#f3f1eb] px-3 py-2.5 text-sm text-[#18392b]"/></label><label className="text-xs font-bold text-[#18392b]/50">Weight (kg)<input type="number" value={profile.weightKg} onChange={e=>setProfile({...profile,weightKg:Number(e.target.value)})} className="mt-1 w-full rounded-xl bg-[#f3f1eb] px-3 py-2.5 text-sm text-[#18392b]"/></label></div></div><div className="rounded-[24px] bg-white p-5 shadow-sm"><div className="font-black">Kitchen equipment</div><div className="mt-4 grid grid-cols-2 gap-2">{applianceOptions.map(a=><button key={a.id} onClick={()=>setAppliances(x=>x.includes(a.id)?x.filter(v=>v!==a.id):[...x,a.id])} className={cx('flex items-center gap-2 rounded-xl border p-3 text-sm font-black',appliances.includes(a.id)?'border-[#18392b] bg-[#dff2df]':'border-[#18392b]/10')}>{a.icon}{a.label}{appliances.includes(a.id)&&<Check size={13} className="ml-auto"/>}</button>)}</div></div></div></>}
    </main>

    {detail&&<div className="fixed inset-0 z-50 overflow-y-auto bg-black/55 p-4" onClick={()=>setDetail(null)}><div className="mx-auto mt-8 max-w-3xl overflow-hidden rounded-[30px] bg-white shadow-2xl" onClick={e=>e.stopPropagation()}>{detail.image?<img src={detail.image} className="h-64 w-full object-cover"/>:<div className="h-64 bg-[#e8e5dc]"/>}<div className="p-6"><div className="flex justify-between gap-4"><div><div className="text-xs font-black uppercase text-[#18392b]/35">{mealTypeOf(detail)} · ≈ {Math.round(caloriesOf(detail)*factor)} kcal your portion</div><h2 className="mt-1 text-3xl font-black">{detail.name}</h2></div><button onClick={()=>setDetail(null)}><X/></button></div><RecipeSteps meal={detail} appliances={appliances}/></div></div></div>}
  </div>
}

function MealCard({meal,open,like,skip,picked}:{meal:Meal;open:()=>void;like:()=>void;skip:()=>void;picked:boolean}){
  return <article className="group overflow-hidden rounded-[24px] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><button onClick={open} className="block w-full text-left"><div className="aspect-[4/3] overflow-hidden bg-[#e8e5dc]">{meal.image?<img src={meal.image} alt={meal.name} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.04]" loading="lazy"/>:<div className="grid h-full place-items-center text-xs font-black uppercase tracking-[.12em] text-[#18392b]/25">Finding photo…</div>}</div><div className="p-4"><div className="flex items-start justify-between gap-2"><h3 className="font-black leading-tight">{meal.name}</h3><span className="shrink-0 rounded-full bg-[#dff2df] px-2 py-1 text-[10px] font-black">≈{caloriesOf(meal)} kcal</span></div><div className="mt-2 text-xs text-[#18392b]/45">{meal.time} min · {meal.cuisine} · {money(meal.cost)}</div>{meal.is_concept&&<div className="mt-2 text-[10px] font-black uppercase tracking-[.12em] text-[#18392b]/35">Recipe generated only if you pick it</div>}{meal.image_page&&<a href={meal.image_page} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} className="mt-2 block text-[10px] font-bold text-[#18392b]/35">Photo{meal.photographer?` by ${meal.photographer}`:''} · Pexels</a>}</div></button><div className="flex gap-2 px-4 pb-4"><button onClick={skip} className="rounded-xl border border-[#18392b]/10 p-2"><X size={14}/></button><button onClick={like} className="flex-1 rounded-xl bg-[#18392b] px-3 py-2 text-xs font-black text-white">{picked?'Remove':'♥ I’d eat this'}</button></div></article>
}
...
function DayCard({day,index,context,update,open}:{day:{day:number;slots:Slot[]};index:number;context:DayContext;update:(p:Partial<DayContext>)=>void;open:(m:Meal)=>void}){return <div className="rounded-[22px] bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><div><div className="text-xs font-black uppercase text-[#18392b]/35">Day {index+1}</div><div className="font-black">{dayNames[index]}</div></div><CalendarDays size={16} className="text-[#18392b]/25"/></div><div className="mt-3 space-y-2">{day.slots.map(s=><button key={s.type} onClick={()=>s.meal&&open(s.meal)} className="flex w-full gap-2 rounded-xl bg-[#f7f5ef] p-2 text-left">{s.meal?.image&&<img src={s.meal.image} className="h-12 w-14 rounded-lg object-cover"/>}<div><div className="text-[9px] font-black uppercase text-[#18392b]/35">{s.type}</div><div className="text-xs font-black">{s.meal?.name}</div></div></button>)}</div></div>}
