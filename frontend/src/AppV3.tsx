...
import {
  AirVent, CalendarDays, Check, ChefHat, ExternalLink, Flame, Home,
  Microwave, PackageCheck, Settings, ShoppingBasket,
  UtensilsCrossed, X,
} from 'lucide-react';

const API = import.meta.env.VITE_API_URL || 'http://localhost:8000/api';
const K = 'bitewise.v3.';
const FALLBACK_IMAGE='https://images.unsplash.com/photo-1546069901-ba9599a7e63c?auto=format&fit=crop&w=1200&q=85';
...
type Product = { id:number|null; name_original:string; brand?:string|null; package_size:number; package_unit:string; price:number|null; product_url?:string|null; ingredient?:string; };
...
type ExtraItem = { id:string; kind:'beverage'|'non-food'; label:string; product:Product|null; dailyUse:number; onHand:number; };
...
function mealTypeOf(m:Meal):MealType{
  ...
  if(/breakfast|oat|pancake|toast|granola|omelette|yogurt|egg/.test(s))return 'breakfast';
...
function imageFor(m:Meal){return m.image||FALLBACK_IMAGE}
function imageFallback(e:React.SyntheticEvent<HTMLImageElement>){const img=e.currentTarget;if(img.src!==FALLBACK_IMAGE)img.src=FALLBACK_IMAGE}
  ...
  return <div className="min-h-screen bg-[#f7f4ee] text-[#17211b]">
    ...
    <main className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6">
      ...
      {tab==='discover'&&<>
        ...
        <div className="mt-3 flex flex-col gap-2 rounded-2xl bg-white p-3 shadow-sm sm:flex-row"><input value={brain} onChange={e=>setBrain(e.target.value)} onKeyDown={e=>e.key==='Enter'&&brain.trim()&&!loading&&generateAI(brain)} className="min-w-0 flex-1 rounded-xl bg-[#f3f1eb] px-4 py-3 text-sm outline-none" placeholder="Fancy but easy tonight · cheap breakfast · use what I have..."/><button disabled={loading} onClick={()=>brain.trim()&&!loading&&generateAI(brain)} className="rounded-xl bg-[#18392b] px-5 py-3 text-sm font-black text-white disabled:opacity-50">{loading?'Thinking…':'Generate'}</button></div>
        ...
        {!visible.length?<div className="mt-5 rounded-[28px] bg-white p-10 text-center"><ChefHat className="mx-auto text-[#18392b]/20"/><div className="mt-3 font-black">{loading?'Generating your options…':'No exact matches yet'}</div>{!loading&&<button onClick={()=>generateAI(`${filter==='all'?'varied':filter} ${mealType} ideas`)} className="mt-4 rounded-full bg-[#18392b] px-5 py-2.5 text-sm font-black text-white">Generate matching meals</button>}</div>:<div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{visible.map(m=><MealCard key={m.id} meal={m} open={()=>!m.is_concept&&setDetail(m)} like={()=>react(m,'like')} skip={()=>react(m,'skip')}/>)}</div>}
      </>}
      ...
      {tab==='today'&&<><Title title="Today" sub={`${dayContexts[todayIndex]?.people||1} person${(dayContexts[todayIndex]?.people||1)>1?'s':''}${dayContexts[todayIndex]?.occasion?` · ${dayContexts[todayIndex].occasion}`:''}`}/><div className="mt-5 space-y-4">{todaySlots.length?todaySlots.map(s=><div key={s.type} className="grid overflow-hidden rounded-[26px] bg-white shadow-sm md:grid-cols-[280px_1fr]"><img src={imageFor(s.meal!)} onError={imageFallback} className="h-56 w-full object-cover md:h-full"/><div className="p-6"><div className="text-xs font-black uppercase tracking-[.16em] text-[#18392b]/35">{s.type}</div><div className="mt-1 text-2xl font-black">{s.meal?.name}</div><div className="mt-2 text-sm text-[#18392b]/45">≈ {Math.round(caloriesOf(s.meal!)*factor)} kcal for your portion · {s.meal?.time} min</div><RecipeSteps meal={s.meal!} appliances={appliances}/></div></div>):<Empty text="No meals planned for today yet."/>}</div></>}
      ...
      {tab==='extras'&&<><Title title="Drinks & extras" sub="Beverages and household items stay outside the meal planner but inside your shopping view."/><div className="mt-4 flex gap-2"><button onClick={()=>setExtraKind('beverage')} className={cx('rounded-full px-4 py-2 text-sm font-black',extraKind==='beverage'?'bg-[#18392b] text-white':'bg-white')}>🥤 Beverages</button><button onClick={()=>setExtraKind('non-food')} className={cx('rounded-full px-4 py-2 text-sm font-black',extraKind==='non-food'?'bg-[#18392b] text-white':'bg-white')}>🧻 Non-food</button></div><div className="mt-3 flex gap-2"><input value={extraQuery} onChange={e=>setExtraQuery(e.target.value)} className="flex-1 rounded-xl bg-white px-4 py-3" placeholder="Search REWE catalog..."/><button onClick={searchExtras} className="rounded-xl bg-[#18392b] px-4 py-3 text-white font-black">Search</button></div><div className="mt-3 grid gap-2 md:grid-cols-2">{extraResults.slice(0,8).map(p=><button key={String(p.id)+p.name_original} onClick={()=>addExtra(p)} className="rounded-xl bg-white p-3 text-left"><div className="font-black text-sm">{p.name_original}</div><div className="text-xs text-[#18392b]/40">{money(p.price)}</div></button>)}</div><div className="mt-5 space-y-3">{extras.map(e=><div key={e.id} className="rounded-2xl bg-white p-4"><div className="flex justify-between"><div className="font-black">{e.label}</div><button onClick={()=>setExtras(x=>x.filter(v=>v.id!==e.id))}><X size={14}/></button></div><div className="mt-3 grid grid-cols-2 gap-2"><label className="text-xs">Daily use<input type="number" value={e.dailyUse} onChange={ev=>updateExtra(e.id,{dailyUse:Number(ev.target.value)})} className="mt-1 w-full rounded-lg bg-[#f3f1eb] px-2 py-2"/></label><label className="text-xs">On hand<input type="number" value={e.onHand} onChange={ev=>updateExtra(e.id,{onHand:Number(ev.target.value)})} className="mt-1 w-full rounded-lg bg-[#f3f1eb] px-2 py-2"/></label></div></div>)}</div></>}

      {tab==='pantry'&&<><Title title="Pantry" sub="What you already own reduces the basket automatically."/><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{pantry.map((p:any)=><div key={p.id} className="rounded-[20px] bg-white p-4 shadow-sm"><div className="font-black capitalize">{p.ingredient}</div><div className="mt-1 text-sm text-[#18392b]/45">{p.quantity} {p.unit} · {p.status}</div></div>)}</div></>}
    ...
    {detail&&<div className="fixed inset-0 z-50 overflow-y-auto bg-black/55 p-4" onClick={()=>setDetail(null)}><div className="mx-auto mt-8 max-w-3xl overflow-hidden rounded-[30px] bg-white shadow-2xl" onClick={e=>e.stopPropagation()}><img src={imageFor(detail)} onError={imageFallback} className="h-64 w-full object-cover"/><div className="p-6"><div className="flex justify-between gap-4"><div><div className="text-xs font-black uppercase text-[#18392b]/35">{mealTypeOf(detail)} · ≈ {Math.round(caloriesOf(detail)*factor)} kcal your portion</div><h2 className="mt-1 text-3xl font-black">{detail.name}</h2></div><button onClick={()=>setDetail(null)}><X/></button></div><RecipeSteps meal={detail} appliances={appliances}/><div className="mt-5"><div className="text-xs font-black uppercase text-[#18392b]/35">Ingredients</div><div className="mt-2 flex flex-wrap gap-2">{detail.ingredients.map(([n,q,u])=><span key={n} className="rounded-full bg-[#f3f1eb] px-3 py-1.5 text-xs font-bold">{n} · {Math.round(q*factor*100)/100} {u}</span>)}</div></div></div></div></div>}

    {altItem&&<div className="fixed inset-0 z-50 bg-black/50 p-4" onClick={()=>setAltItem(null)}><div className="mx-auto mt-20 max-w-xl rounded-[26px] bg-white p-5" onClick={e=>e.stopPropagation()}><div className="flex justify-between"><div className="font-black">Choose alternative</div><button onClick={()=>setAltItem(null)}><X size={18}/></button></div><div className="mt-4 space-y-2">{alternatives.slice(0,10).map(p=><button key={String(p.id)} onClick={()=>chooseAlternative(p)} className="w-full rounded-xl bg-[#f5f4ef] p-3 text-left"><div className="font-black text-sm">{p.name_original}</div><div className="text-xs text-[#18392b]/40">{money(p.price)}</div></button>)}</div></div></div>}
  </div>
}

function MealCard({meal,open,like,skip,picked=false}:{meal:Meal;open:()=>void;like:()=>void;skip:()=>void;picked?:boolean}){
  return <article className="group overflow-hidden rounded-[24px] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><button onClick={open} className="block w-full text-left"><div className="aspect-[4/3] overflow-hidden bg-[#e8e5dc]"><img src={imageFor(meal)} alt={meal.name} onError={imageFallback} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.04]" loading="lazy"/></div><div className="p-4"><div className="flex items-start justify-between gap-2"><h3 className="font-black leading-tight">{meal.name}</h3><span className="shrink-0 rounded-full bg-[#dff2df] px-2 py-1 text-[10px] font-black">≈{caloriesOf(meal)} kcal</span></div><div className="mt-2 text-xs text-[#18392b]/45">{meal.time} min · {meal.cuisine} · {money(meal.cost)}</div>{meal.is_concept&&<div className="mt-2 text-[10px] font-black uppercase tracking-[.12em] text-[#18392b]/35">Recipe generated only if you pick it</div>}</div></button><div className="flex gap-2 px-4 pb-4"><button onClick={skip} className="rounded-xl border border-[#18392b]/10 p-2"><X size={14}/></button><button onClick={like} className="flex-1 rounded-xl bg-[#18392b] px-3 py-2 text-xs font-black text-white">{picked?'Remove':'♥ I’d eat this'}</button></div></article>
}
