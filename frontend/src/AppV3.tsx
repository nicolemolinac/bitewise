...
export default function AppV3(){
  ...
  return <div className="min-h-screen bg-[#f7f4ee] text-[#17211b]">
    ...
    <main className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6">
      ...
      {tab==='discover'&&<>
        <div className="grid gap-3 rounded-[26px] bg-[#18392b] p-5 text-white lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="text-xs font-black uppercase tracking-[.18em] text-white/45">AI food decision engine</div><h1 className="mt-1 text-2xl font-black">What do you feel like eating?</h1><p className="mt-1 text-sm text-white/55">Choose a meal type or tell Bitewise what you want. Your pantry, REWE catalog and appliances shape the ideas.</p></div><div className="text-right"><div className="text-xs text-white/40">Adaptive portion</div><div className="text-xl font-black">{factor.toFixed(2)}×</div></div></div>
...
function MealCard({meal,appliances,open,like,skip,picked=false}:{meal:Meal;appliances:string[];open:()=>void;like:()=>void;skip:()=>void;picked?:boolean}){
  return <article className="group overflow-hidden rounded-[24px] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><button onClick={open} className="block w-full text-left"><div className="aspect-[4/3] overflow-hidden bg-[#e8e5dc]">{meal.image&&<img src={meal.image} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.04]"/>}</div><div className="p-4"><div className="flex items-start justify-between gap-2"><h3 className="font-black leading-tight">{meal.name}</h3><span className="shrink-0 rounded-full bg-[#dff2df] px-2 py-1 text-[10px] font-black">≈{caloriesOf(meal)} kcal</span></div><div className="mt-2 text-xs text-[#18392b]/45">{meal.time} min · {meal.cuisine} · {money(meal.cost)}</div></div></button><div className="flex gap-2 px-4 pb-4"><button onClick={skip} className="rounded-xl border border-[#18392b]/10 p-2"><X size={14}/></button><button onClick={like} className="flex-1 rounded-xl bg-[#18392b] px-3 py-2 text-xs font-black text-white">{picked?'Remove':'♥ I’d eat this'}</button></div></article>
}
