...
export default function AuthGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Render uses private HTTP Basic Auth around the whole app. When Supabase is not
    // configured we still hydrate the shared Postgres-backed user state before the
    // UI mounts, so laptop and phone keep the same Bitewise choices and settings.
    if (!supabase) {
      hydrateCloudState()
        .catch((e: any) => { if (alive) setError(e?.message || 'Cloud sync failed'); })
  ...
  if (!ready) return <div className="min-h-dvh grid place-items-center bg-[#f6f4ef] text-[#171916]"><div className="text-center"><div className="mx-auto h-12 w-12 animate-pulse rounded-2xl bg-[#203d2f]"/><div className="mt-4 text-sm font-black">Syncing Bitewise…</div>{error&&<div className="mt-3 text-xs font-bold text-red-700">{error}</div>}</div></div>;
